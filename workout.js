...
let _allExercises = []; // cached for picker

// ══════════════════════════════════════════════════════════════
// EXERCISE PICKER — fullscreen mobile modal
// ══════════════════════════════════════════════════════════════
...
function renderPickerChips() {
  ...
  if (!chips) return;
  chips.innerHTML = `<div class="picker-chip ${!window._pickerMuscle?'active':''}"
    onclick="setPickerMuscle('')">Tutti</div>` +
    window.MUSCLES.map(m => `<div class="picker-chip ${window._pickerMuscle===m?'active':''}"
      onclick="setPickerMuscle('${m}')">${m}</div>`).join('');
}
...
function renderPickerList(query, muscle) {
  ...
  if (!filtered.length) {
    list.innerHTML = '<div class="empty">Nessun esercizio trovato</div>';
    return;
  }
  list.innerHTML = filtered.map(e => `
    <div class="picker-item" onclick="pickerSelect('${e.id}','${esc(e.name)}','${e.muscle_group}','${e.emoji||'🏋️'}')">
      <div class="picker-item-emoji">${e.emoji || '🏋️'}</div>
      <div>
        <div class="picker-item-name">${esc(e.name)}</div>
        <div class="picker-item-muscle">${e.muscle_group}</div>
      </div>
    </div>`).join('');
}
...
function pickerSearch() {
  ...
  renderPickerList(q, window._pickerMuscle || '');
}
...
async function renderFreeWorkout() {
  // Reset if coming fresh
  if (!freeWorkout.sessionId) {
  ...
  <div class="flex-between">
    <div class="page-title">Allenamento Libero <span class="page-sub">Senza scheda</span></div>
    <div id="fw_timer" style="font-family:'Bebas Neue',sans-serif;font-size:1.6rem;color:var(--accent);${isActive?'':'display:none'}">00:00</div>
  </div>

  ${!isActive ? `
  <div class="card">
    <div class="card-title">Nuova Sessione</div>
    <div class="fg"><label>Note iniziali</label>
      <textarea class="inp" id="fw_note" rows="2" placeholder="Obiettivo di oggi, come ti senti..."></textarea>
    </div>
    <button class="btn-primary btn-full" onclick="startFreeWorkout()">▶ Inizia Allenamento</button>
  </div>` : `
  <div class="card" style="border-left:3px solid var(--accent)">
    <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:0.8rem">
      <div>
        <div style="font-family:'Barlow Condensed',sans-serif;font-size:0.72rem;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:var(--accent)">Sessione in corso</div>
        <div style="font-size:0.75rem;color:var(--muted);margin-top:0.2rem">${freeWorkout.exercises.length} esercizi aggiunti</div>
      </div>
      <div class="flex-gap">
  ...
  <div style="margin-top:0.5rem">
    <button class="btn-primary btn-full" onclick="openExPicker(addToFreeWorkout)">+ Aggiungi Esercizio</button>
  </div>
...
function startFreeWorkout() {
  ...
  freeWorkout = { sessionId: 'pending', exercises: [], startTime: Date.now(), note };
  renderFreeWorkout();
}
...
function addToFreeWorkout(exId, name, muscleGroup, emoji) {
  const id = uid();
  freeWorkout.exercises.push({ id, exId, name, muscleGroup, emoji, serie:3, reps:10, weight:0, done:false, comment:'' });
  renderFWExercises();
...
function renderFWExercises() {
  ...
  if (!freeWorkout.exercises.length) {
    container.innerHTML = `<div class="empty" style="margin:0.5rem 0">
      Nessun esercizio — tocca <strong style="color:var(--accent)">+ Aggiungi Esercizio</strong>
    </div>`;
    return;
  }
  ...
  <div class="ex-log-card ${e.done?'done':''}" id="fwex_${e.id}">
    <div class="ex-log-header">
      <div class="ex-log-info">
        <div class="ex-mini-thumb" style="width:42px;height:42px;font-size:1.4rem;flex-shrink:0">${e.emoji||'🏋️'}</div>
        <div style="flex:1;min-width:0">
          ...
          <div class="fw-set-row">
            <div class="fw-set-col">
              ...
              <select class="dd" style="width:58px;padding:0.45rem 0.4rem" onchange="updateFWEx('${e.id}','serie',this.value)">
                ${window.SERIE_OPTS.map(s=>`<option ${e.serie==s?'selected':''}>${s}</option>`).join('')}
              </select>
            </div>
            <div class="fw-set-col">
              <span class="fw-set-label">Reps</span>
              <select class="dd" style="width:62px;padding:0.45rem 0.4rem" onchange="updateFWEx('${e.id}','reps',this.value)">
                ${window.REPS_OPTS.map(r=>`<option ${e.reps==r?'selected':''}>${r}</option>`).join('')}
              </select>
            </div>
            <div class="fw-set-col">
              <span class="fw-set-label">Peso kg</span>
              <select class="dd" style="width:72px;padding:0.45rem 0.4rem" onchange="updateFWEx('${e.id}','weight',this.value)">
          ...
          <div class="fg" style="margin-top:0.6rem;margin-bottom:0">
            <label>Note</label>
            <input class="inp" value="${esc(e.comment)}" placeholder="Note..."
...
async function renderToday() {
  ...
  set(`<div class="flex-between">
    <div class="page-title">Oggi <span class="page-sub">${window.DAYS[window.DKEYS.indexOf(dk)]}</span></div>
    ${exs.length ? `<span class="tag ${done===exs.length&&done?'accent':''}">${done}/${exs.length}</span>` : ''}
  </div>
  ${!asgn
    ? `<div class="info-box">Nessuna scheda assegnata.<br>Puoi fare un <a href="#" onclick="router('free_workout')" style="color:var(--accent)">allenamento libero</a>!</div>`
    : !exs.length
